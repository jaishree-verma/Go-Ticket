...
export const processAgentMessage = async (userMessage = '', agentState = {}, options = {}) => {
 ...
 agentState,
 statusTrace: [],
 text: ` **Unauthorized:** Ticket **${ticketId}** is registered to another passenger. You can only cancel bookings associated with your registered account.`,
 chips: ['Find buses from Kanpur to Delhi tomorrow']
 };
 }
 }
 ...
 responseText = ` **Goal Analysis:** ${decision.conflictSummary}.\n\nHere are the closest matching alternatives:\n\n`;
 ...
 btnText: 'Open Interactive Seat Map ',
 routeState: {
 from: params.source,
 ...
 text: ` **Seat Selection Limit:** You have requested booking for **${requiredPassengers} passenger${requiredPassengers > 1 ? 's' : ''}**, but attempted to select ${targetSeats.length} seats (${targetSeats.join(', ')}).\n\nPlease select exactly **${requiredPassengers} seat${requiredPassengers > 1 ? 's' : ''}**.`,
 seatMap: {
 busId: selectedBus.id,
 busName: selectedBus.busName,
 operator: selectedBus.operator,
 ...
 text: ' **Sign In Required:** Please sign in before I complete your booking. Your selected seats and details are saved.',
 actionCard: {
